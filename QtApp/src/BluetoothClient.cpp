...
void Client::start(const QBluetoothServiceInfo &info)
{
    ...
    connect(socket.get(), &QBluetoothSocket::connected, [=]{
        emit connected({info.device().name(), info.device().address().toString()});
    });
    socket->connectToService(info);
}
void Client::start(const QString &address, uint16_t port)
{
    createSocket();
    connect(socket.get(), &QBluetoothSocket::connected, [=]{
        emit connected({socket->peerName(), socket->peerAddress().toString()});
    });
    QBluetoothAddress btAddress(address);
    socket->connectToService(btAddress, port);
}
